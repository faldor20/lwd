...
open Bonsai.Cont
module Bonsai_base = Bonsai
module Bonsai = Bonsai.Cont
open Bonsai.Let_syntax

module Focus : sig
  type var = int Bonsai.Expert.Var.t
  type handle
  ...
  type handle = var * status Bonsai.t

  let make () : handle =
    let v = Bonsai.Expert.Var.create 0 in
    v, v |> Bonsai.Expert.Var.value |> Bonsai.map ~f:(fun i -> Handle (i, v))
  ;;

  let empty : status = Empty
  let status (h : handle) : status Bonsai.t = snd h
  ...
  let release_reversable ((v, _) : handle) =
    ...
    if Expert.Var.get v > 0 then focus_stack |> Stack.pop |> request_var
  ;;
...
module Ui = struct
  ...
  let keyboard_area ?focus f t : t =
    ...
    in
    { t with desc = Focus_area (t, (fun key->let a,b=(f key)in a|>Option.iter Effect.Expert.handle; b)); focus }
  ;;
  ...
  let hcat xs = reduce pack_x xs
  let vcat xs = reduce pack_y xs
  let zcat xs = reduce pack_z xs
...
module Renderer = struct
  ...
  let solve_focus ui i =
    let rec aux ui =
      ...
      | Focus.Handle (_, v) -> Expert.Var.set v 0
    ...
    let rec aux n =
      ...
      then true
        ...
        then aux (n - 1)
          ...
          failwith
            (Printf.sprintf
               "failed with inputs a:%d sa:%d b:%d sb:%d total:%d mA:%d mB:%d\n"
               a
               sa
               b
               sb
...
module Ui_loop = struct
  ...
  let step ?(process_event = true) ?(timeout = -1.0) ~renderer term root =
    ...
    let image =
      let rec stabilize () =
        let tree = Bonsai_driver.result root in
        Renderer.update renderer size tree;
        let image = Renderer.image renderer in
        Bonsai_driver.flush root;
        Bonsai_driver.trigger_lifecycles root;
        (* TODO: I think this always indicates that we need to re-render, but I'm not totally sure*)
        if Bonsai_driver.has_after_display_events root then stabilize () else image
      in
      stabilize ()
    in
  ...
  (* Flush executes the effects or propegates state*)

  let run_with_term term ?tick_period ?(tick = ignore) ~renderer quit t =
    let clock = Time_source.create ~start:(Core.Time_ns.now ()) in
    let root = Bonsai_driver.create ~clock t in
    let rec loop () =
      let quit = Expert.Var.get quit in
      if not quit
      then (
    ...
    in
    loop ();
    ignore (Bonsai_driver.flush root)
  ;;
  ...
  let run
    ...
    let quit =
      match quit with
      | Some quit -> quit
      | None -> Bonsai.Expert.Var.create false
    in
    let t graph =
      let%arr ui = t graph in
      ui
      |> Ui.event_filter (function
        | `Key (`ASCII 'Q', [ `Ctrl ]) when quit_on_ctrl_q ->
          Expert.Var.set quit true;
          `Handled
